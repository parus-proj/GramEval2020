...
from allennlp.data.fields import Field, TextField, SequenceLabelField, MetadataField, ListField, LabelField
from allennlp.data.instance import Instance
from allennlp.data.token_indexers import SingleIdTokenIndexer, TokenIndexer
from allennlp.data.tokenizers import Token, Tokenizer

from train_ml.corpus_iterator import CorpusIterator, Sentence as CorpusSentence
...
class UDDatasetReader(DatasetReader):
    ...
    def _read(self, file_path: str):
        print("ITLOG: processing %s" % file_path)
...
#         print("ITLOG: in text_to_instance")
        ...
        if sentence.pos_tags:
            if not self._skip_labels:
                ...
                fields['pos_tags'] = ListField( [ListField( [LabelField(label, 'pos_value_tags') for label in token_labels ] ) for token_labels in sentence.pos_tags] )
                #pd = fields['pos_tags'].get_padding_lengths()
                #print(pd)
        ...
        if sentence.grammar_values and not self._skip_labels:
            ...
            fields['grammar_values'] = ListField( [ListField( [LabelField(label, 'grammar_value_tags') for label in token_labels ] ) for token_labels in sentence.grammar_values] )
