...
from allennlp.data.fields import SequenceLabelField, ListField, LabelField
...
class LemmatizeHelper(object):
    ...
    _EMPTY_RULE_PLACEHOLDER = LemmatizeRule(append_suffix='EMPTY', clear=True)
    ...
    def fit(self, data, min_freq=3):
        ...
        for instance in tqdm(data):
            ...
            for word, vlemma in zip(words, lemmas):
                for lemma in vlemma:
        ...
        self._lemmatize_rules = {
            ...
            self._EMPTY_RULE_PLACEHOLDER:   self.UNKNOWN_RULE_INDEX + 1,
            LemmatizeRule():                self.UNKNOWN_RULE_INDEX + 2
        }
        ...
        logger.info( 'Lemmatize rules count = {}, did not cover {:.2%} of words'.format( len(self._lemmatize_rules), skipped_count / total_count ) )
    ...
    def get_rule_index(self, word, lemma):
        ...
        if lemma == 'EMPTY':
            return self.UNKNOWN_RULE_INDEX+1  # хардкод для спец.правила
    ...
    def get_rule_indices(self, instance):
        ...
        return [ [self.get_rule_index(word, lemma) for lemma in vlemma ] for word, vlemma in zip(words, lemmas)]
    ...
    def predict_lemmatize_rule(word: str, lemma: str):
        ...
        if result.cut_prefix + result.cut_suffix == len(word):
            result = LemmatizeRule(append_suffix=result.append_suffix, clear=True)  # правило полного замещения словоформы леммой: их--они, люди--человек и т.п.
    ...
    def apply_to_instances(self, instances):
        for instance in tqdm(instances, 'Lemmatizer apply_to_instances'):
            ...
            field = ListField( [ListField( [LabelField(label, 'lemma_index_tags', True) for label in token_labels ] ) for token_labels in lemmatize_rule_indices] )
