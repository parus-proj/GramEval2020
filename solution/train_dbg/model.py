...
class DependencyParser(Model):
    ...
    def decode(self, output_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        top_lemmas_prob = output_dict.pop("top_lemmas_prob").cpu().detach().numpy()
        top_gramms = output_dict.pop("top_gramms").cpu().detach().numpy()
        top_gramms_prob = output_dict.pop("top_gramms_prob").cpu().detach().numpy()
        top_heads = output_dict.pop("top_heads").cpu().detach().numpy()
        top_deprels = output_dict.pop("top_deprels").cpu().detach().numpy()
        top_dr_probs = output_dict.pop("top_deprels_prob").cpu().detach().numpy()

        assert len(head_tags) == len(heads) == len(lengths) == len(predicted_gram_vals) == len(predicted_lemmas) == len(top_lemmas) == len(top_gramms)
        ...
        for instance_index in range(len(head_tags)):
            ...
            decoded_gram_vals.append([
                self.vocab.get_token_from_index(gram_val, "grammar_value_tags") for gram_val in gram_vals
            ])
            ...
            for w_i in range(len(words)):
                ...
                for top_i in range(self.TopNCnt):
                    word_top_lemmas.append( self.lemmatize_helper.lemmatize(words[w_i], top_lemmas[instance_index][w_i][top_i]) )
            ...
            for w_i in range(len(words)):
                word_top_gramms = []
                for top_i in range(self.TopNCnt):
                    word_top_gramms.append( self.vocab.get_token_from_index(top_gramms[instance_index][w_i][top_i], "grammar_value_tags") )
            ...
            for w_i in range(len(words)):
                ...
                for top_i in range(self.TopNCnt-1):
                    ...
                    lbl = self.vocab.get_token_from_index(floor(top_i_rel), "head_tags")
                    word_top_deprels.append( str(top_i_head_no) + " " + lbl + " ({:.5f})".format(top_i_prb) )
    ...
    def _parse(self,
        ...
        top_gramms_indices = (-grammar_value_logits).argsort(-1)[:,:,:self.TopNCnt]
        ...
        if self.training or not self.use_mst_decoding_for_validation:
            ...
            synt_prediction, benrg = self._mst_decode(head_tag_representation, child_tag_representation, attended_arcs, mask)
        ...
        benrgf = torch.flatten(benrg, start_dim=1, end_dim=2).permute(0, 2, 1)    # склеивает тип синт. отношения с индексом родителя
        top_deprels_indices = (-benrgf).argsort(-1)[:,:,:self.TopNCnt]   # отбираем наилучшие комбинации
        ...
        top_deprels_indices = torch.div(top_deprels_indices, seqlen)  # torch.floor не срабатывает
